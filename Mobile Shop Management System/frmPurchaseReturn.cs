...
namespace Mobile_Shop_Management_System
{
    public partial class frmPurchaseReturn : Form
    {
        ...
        {
            //get last id 
            string sql = @"SELECT id FROM  tblPurchaseInvoice ORDER BY id DESC LIMIT 1";
            ...
            if (e.KeyCode == Keys.Enter)
            {
                ...
                if (imieTextBox.TextLength > 0)
                {
                    ...
                    else
                    {
                        ...
                        adapt = new SQLiteDataAdapter("select * from tblPurchaseInvoiceItem where purchase_itemid=" + Convert.ToInt64(dt.Rows[0]["id"].ToString()) + "", con);
                        adapt.Fill(dt2);
                        con.Close();
                        ...
                        else
                        {
                            ...
                            adapt = new SQLiteDataAdapter("select * from tblPurchaseInvoice where id=" + Convert.ToInt64(dt2.Rows[0]["sale_itemid"].ToString()) + "", con);
                            adapt.Fill(dt3);
                            con.Close();
        ...
        private void AddButton_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrWhiteSpace(imieTextBox.Text))
            {
                con.Open();
                DataTable dt1 = new DataTable();
                adapt = new SQLiteDataAdapter("select purchase.id as id ,sale.sale_price as price,item.received as received, item.return as return from tblPurchase as purchase  inner join tblPurchaseInvoiceItem as sale on purchase.id=sale.purchase_itemid inner join tblPurchaseInvoice as item on sale.sale_itemid=item.id where purchase.imie='" + imieTextBox.Text.ToString() + "'", con);
                adapt.Fill(dt1);
                con.Close();
                ...
                if (dt1.Rows.Count > 0)
                {
                    ...
                    /*8
                    SQLiteCommand cmd1 = new SQLiteCommand("update tblPurchaseInvoice set total=@total,received=@received,balance=@balance,return=@return  where id=@id ", con);
                    con.Open();
        ...
        public void refreshGridView()
        {
            con.Open();
            DataTable dt = new DataTable();
            adapt = new SQLiteDataAdapter("SELECT tblPurchase.id as ID,tblPurchase.imie as IMIE, tblPurchaseInvoiceItem.sale_price as price,tblPurchase.description as Description FROM tblPurchase INNER JOIN tblPurchaseInvoiceItem ON  tblPurchaseInvoiceItem.purchase_itemid=tblPurchase.id where tblPurchaseInvoiceItem.sale_itemid=" + invoiceid, con);
            adapt.Fill(dt);
        ...
        private void dataGridView1_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            if (dataGridView1.Columns[e.ColumnIndex].Name == "Price")
            {
                ...
                else
                {
                    ...
                    cmd1.Parameters.AddWithValue("@id", id);
                    cmd1.Parameters.AddWithValue("@sale_itemid", invoiceid);
                    cmd1.ExecuteNonQuery();
                    con.Close();
            ...
            {
                string description = dataGridView1.Rows[e.RowIndex].Cells["Price"].Value.ToString();
                long id = Convert.ToInt64(dataGridView1.Rows[e.RowIndex].Cells["ID"].Value.ToString());


                SQLiteCommand cmd = new SQLiteCommand("update tblPurchase set description=@description where id=@id ", con);
                con.Open();
        ...
        public void generateSuggestionsforInvNo()
        {
            ...
            SQLiteCommand cmnd1 = con.CreateCommand();
            cmnd1.CommandType = CommandType.Text;
            cmnd1.CommandText = "SELECT * FROM tblPurchaseInvoice";
            SQLiteDataReader Reader;
            Reader = cmnd1.ExecuteReader();
        ...
        private void invoiceTextBox_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                if (invoiceTextBox.TextLength > 0)
                {
                    con.Open();
                    DataTable dt = new DataTable();
                    adapt = new SQLiteDataAdapter("select * from tblPurchaseInvoice where id=" + Convert.ToInt64(invoiceTextBox.Text.ToString()) + "", con);
                    adapt.Fill(dt);
                    con.Close();
                    if (dt.Rows.Count == 0)
                    {
        ...
        public void fillDataGridView()
        {
            ...
            SQLiteCommand cmnd1 = con.CreateCommand();
            cmnd1.CommandType = CommandType.Text;
            cmnd1.CommandText = "SELECT tblPurchase.id as ID,tblPurchase.imie as IMIE, tblPurchaseInvoiceItem.sale_price as price,tblPurchase.description as Description FROM tblPurchase INNER JOIN tblPurchaseInvoiceItem ON  tblPurchaseInvoiceItem.purchase_itemid=tblPurchase.id where tblPurchaseInvoiceItem.sale_itemid=" + invoiceid;
            SQLiteDataReader Reader;
            Reader = cmnd1.ExecuteReader();
