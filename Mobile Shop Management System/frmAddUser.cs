    ...
    public partial class frmAddUser : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (Convert.ToInt64(accountidTextBox.Text) == lastId)
            {
                ...
                else
                {
                    if (EverythingOK())
                    {
                        ...
                        cmd = new SQLiteCommand("insert into tblUser(name,username,created_at,password) values(@name,@username,@created_at,@password)", con);
                        con.Open();
                    ...
                    {
                        MessageBox.Show("Username Matched ! Can't Update");
            ...
            else
            {
                if (EverythingOK())
                {
                    SQLiteCommand cmd = new SQLiteCommand("update tblUser set username=@phone ,name=@name,password=@cnic where id=@id", con);
                    con.Open();
                    cmd.Parameters.AddWithValue("@phone", usernameTextBox.Text);
                    cmd.Parameters.AddWithValue("@name", fullnameTextBox.Text);
                    cmd.Parameters.AddWithValue("@cnic", passwordTextBox.Text);
        ...
        private void DisplayData()
        {
            ...
            else
            {
                ...
                //get first id
                string sql1 = @"SELECT id FROM  tblUser ORDER BY id ASC LIMIT 1";
        ...
        */
        ...
        public Boolean getTblPurchasePreviousRow(long id)
        {
            ...
            DataTable dt = new DataTable();
            adapt = new SQLiteDataAdapter("select * from tblUser where id=" + id, con);
            con.Open();
            adapt.Fill(dt);
            if (dt.Rows.Count == 0)
            {
        ...
        public bool EverythingOK()
        {
            ...
            */
            ...
            adapt1 = new SQLiteDataAdapter("select * from tblUser where username='" + usernameTextBox.Text.ToString() + "' and id!=" + Convert.ToInt64(accountidTextBox.Text.ToString()), con);
            con.Open();
            adapt1.Fill(dt1);
