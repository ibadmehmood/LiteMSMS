...
namespace Mobile_Shop_Management_System
{
    public partial class frmSaleInvoice : Form
    {
        ...
        {
            //get last id 
            string sql = @"SELECT seq FROM  sqlite_sequence where name='"+"tblSaleInvoice'";
        ...
        {
            SQLiteCommand cmd = new SQLiteCommand("insert into tblSaleInvoice(date) values(@date)", con);
            con.Open();
        ...
        {
            SQLiteCommand cmd = new SQLiteCommand("insert into tblSaleInvoiceItem(purchase_itemid,sale_itemid,sale_price,date) values(@purchase_itemid,@sale_itemid,@sale_price,@date)", con);
            con.Open();
        ...
        public void isFirstEntry()
        {
            con.Open();
            DataTable dt = new DataTable();
            adapt = new SQLiteDataAdapter("SELECT * from tblSaleInvoice where id=" + currentid, con);
            adapt.Fill(dt);
            con.Close();
            if (dt.Rows.Count == 0)
            {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
                 ...
                 if (Convert.ToInt64(balanceTextBox.Text.ToString()) < 0)
                {
                    ...
                    long currentbalance = (total - Convert.ToInt64(receivedTextBox.Text.ToString()))+Convert.ToInt64(returnbalanceTextBox.Text.ToString());
                     ...
                     else
                     {
                         ...
                         con.Open();
                         cmd6.Parameters.AddWithValue("@type", "sale");
                         ...
                         cmd6.Parameters.AddWithValue("@receipt", Convert.ToInt64(receivedTextBox.Text.ToString()) - Convert.ToInt64(returnbalanceTextBox.Text.ToString()));
            ...
            else
            {
                ...
                SQLiteCommand cmd = new SQLiteCommand("update tblSaleInvoice set total=@total ,return=@return,received=@received,balance=@balance,status=@status,account_id=@account_id where id=@id", con);
                con.Open();
                cmd.Parameters.AddWithValue("@total", total);
